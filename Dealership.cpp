...
Dealership::Dealership(const vector<Vehicle>& vehicles) : inventory(vehicles) {}

Dealership::Dealership() {
    inventory = parseVehicles(vehicleData);
    cout << "\n--- Dealership Created ---\n";
...
vector<Vehicle> Dealership::parseVehicles(const string& csvData) {
    ...
    while (getline(ss, line)) {
        ...
        if (cells.size() != 6) {
            cerr << "Skipping malformed line (expected 5 columns, got " << cells.size() << "): " << line << endl;
            continue;
        }
        ...
        try {
            ...
            int price = stoi(cells[5]);

            // Create and store the new Vehicle object
            vehicles.emplace_back(vin, year, make, model, color, price);
